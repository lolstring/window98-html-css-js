...
xepOnline.Formatter = {
	...
	*/
	__format: function(ElementIDs, options) {
		...
		options.attach = (options.attach === undefined) ? false : options.attach;
		options.fileID = (options.fileID === undefined) ? 'fileid-00000' : options.fileID;
...
// 			if(options.attach){
// 				var fd = new FormData(document.querySelector("#temp_post"));
// 				console.log(fd);
// 				$.ajax({
//  					 url: xepOnline.Formatter.xep_chandra_service_AS_PDF,
//   						type: "POST",
// 						data: fd,
...
// 			}
			...
			//  	jQuery('#temp_post').submit(function(e){
			//  	 var self=this;
   // 				 var formData = new FormData('#temp_post');
   // 				 console.log(formData);
   // 				 var formURL = xepOnline.Formatter.xep_chandra_service_AS_PDF;	
   // 				 console.log(e);
   // 				 $.ajax({
   // 	        		url: formURL,
   // 	       		    type: 'POST',
   ...
   //        		  },
   //        		  error: function(){
   //         		    alert("External pdf Server Down");
   //         		     $('#'+options.attach + ' .attachment [file-id="'+options.fileID+']"').remove();
   //         		     $('#temp-content').remove();
   //         		 }
 		// 	     });
